...
$upload_dir = "/clients/source/";
$current_path = "";
$current_path_thumbs = '../../../../../clients/thumbs/';

if(!empty($base_dir)){
    $array_dir = explode("assets", $base_dir);
    $array_path= explode("/", $base_dir);
    if(count($array_path))
    {
        foreach ($array_path as $r)
        {
            if(!empty($r)){
                $current_path .= "../";
            }
    ...
    $upload_dir = $array_dir[0]."clients/source/";
    $current_path .= substr($upload_dir, 1);
}
/*
...
echo '<br>';
echo "current_path_thumbs : ".$current_path_thumbs;
exit;
*/
